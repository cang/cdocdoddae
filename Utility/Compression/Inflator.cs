...
namespace SiGlaz.Utility.Compression
{
	...
	{
		private static Inflator _instance;
		...
		public static Inflator Instance
		{
			get
			{
				if (_instance != null)
					return _instance;
				_instance = new Inflator();
				return _instance;
			}
		}
		...
		private unsafe int GetNBits(int n)
		{
			...
			while (bits < n)
				...
				else if (_srcIndex < _breakOffset)
				{
					...
					MCException.ThrowReadBeyondTheEndException();
			_hold = hold >> n;
			_bits = bits - n;
			return hold & ((1 << n) - 1);
		}
		...
		{
			int n, m, p, i, code;
			Utils.Fill(0, _pBitCount, Consts.MaxChLenBits + 1);
			...
			Utils.Fill(0, _pChLenTree, Consts.ChLenTreeSize);
			_pNextCode[1] = 0;
			...
			for (i = 0; i < Consts.ChLenCount; i++)
			{
				...
				if (n == 0)
					continue;
				m = _pNextCode[n];
				code = (int)Utils.ReverseBits((uint)m, n);
				_pNextCode[n] = m + 1;
				p = 1;
				while (true)
				{
		...
		private unsafe void LoadCharDistLengths(int count)
		{
			int c, lastLen = 0;
			int* p = _pBitLen;
			Utils.Fill(0, _pBitCount, Consts.MaxBits + 1);
			while (count > 0)
			{
		...
		{
			int n, m, p, code;
			Utils.Fill(0, _pBitLen, Consts.CharCount);
			LoadCharDistLengths(GetNBits(6) + 257);
			Utils.Fill(0, _pCharTree, Consts.CharTreeSize);
			_pNextCode[1] = 0;
			_pNextCode[2] = n = _pBitCount[1] << 1;
			_pNextCode[3] = n = (n + _pBitCount[2]) << 1;
			_pNextCode[4] = n = (n + _pBitCount[3]) << 1;
		...
		{
			int n, m, p, code;
			Utils.Fill(0, _pBitLen, Consts.DistCount);
			LoadCharDistLengths(GetNBits(6) + 1);
			Utils.Fill(0, _pDistTree, Consts.DistTreeSize);
			_pNextCode[1] = 0;
			_pNextCode[2] = n = _pBitCount[1] << 1;
			_pNextCode[3] = n = (n + _pBitCount[2]) << 1;
			_pNextCode[4] = n = (n + _pBitCount[3]) << 1;
			...
			fixed (byte* pSrcBytes = &sourceBytes[sourceIndex], pDstBytes = &destinationBytes[destinationIndex])
			{
				...
				if (byteCount <= 0)
				{
					...
					if (destinationBytes.Length - destinationIndex < byteCount)
						MCException.ThrowNoPlaceToStoreDecompressedDataException();
					if (byteCount > 0)
					...
					while (_outCounter > 0)
					{
						...
						while (_inCounter > 0 && _outCounter > 0)
						{
							...
							else
							{
								...
								Utils.CopyBytes(_pDstBytes - distance, _pDstBytes, length);
								_outCounter -= length;
								_pDstBytes += length;
							}
						}
