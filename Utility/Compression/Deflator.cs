...
{
	// Deflator class (not thread-safe)

	public class Deflator
	{
		private static Deflator _instance;
		...
		public static Deflator Instance
		{
			get
			{
				if (_instance != null)
					return _instance;
				_instance = new Deflator();
				return _instance;
			}
		}
		...
		private unsafe int PrepareCharLengths()
		{
			...
			Utils.Fill(0, _pCharBitCount, Consts.MaxBits + 1);
			int overf = 0;
			int bitAmount = 0;
			int n, m;
			int* pT = _pTree + i;
		...
		private unsafe int PrepareDistLengths()
		{
			...
			Utils.Fill(0, _pDistBitCount, Consts.MaxBits + 1);
			int overf = 0;
			int bitAmount = 0;
			int n, m;
			int* pT = _pTree + i;
		...
		private unsafe int FillCharBitLengths()
		{
			...
			Utils.Fill(0, _pCharBitLenEx, _charLenCount);
		...
		private unsafe int FillDistBitLengths()
		{
			...
			Utils.Fill(0, _pDistBitLenEx, _distLenCount);
			int result = 0;
			do
			{
				int m = *pDad;
		...
		private unsafe void FillChLenFreqCodes()
		{
			...
			Utils.Fill(0, pFreq, Consts.ChLenCount);
			int i;
			int* pLen = _pCharBitLen;
		...
		private unsafe int PrepareChLenLengths()
		{
			...
			Utils.Fill(0, _pChLenBitCount, Consts.MaxChLenBits + 1);
			int overf = 0;
			int bitAmount = 0;
			int n, m;
			int* pT = _pTree + i;
		...
		private unsafe void GenerateCodes()
		{
			...
			for (i = Consts.ChLenCount; i > 0; i--)
			{
				...
				{
					m = p[n];
					*pFC = (int)Utils.ReverseBits((uint)m, n);
					p[n] = m + 1;
				}
				pBD++;
				pFC++;
		...
		private unsafe bool FlushBuffer()
		{
			...
			Utils.Fill(0, _pCharFreqCode, Consts.CharCount);
			Utils.Fill(0, _pDistFreqCode, Consts.DistCount);
			_exBitCount = 0;
		...
		private unsafe int CompressCore()
		{
					   ...
					   pChLenExBitLength = &Consts.ChLenExBitLength[0])
			{
				...
				Utils.Fill(Consts.InitPosValue, _pHash, _hashSize);
				Utils.Fill(0, _pCharFreqCode, Consts.CharCount);
				Utils.Fill(0, _pDistFreqCode, Consts.DistCount);
				_blockStart = 0;
				_bufLength = 0;
			...
			CompressionLevel compressionLevel, int beforeGap, int afterGap)
		{
			...
			if (sourceBytes == null)
				MCException.ThrowArgumentNullException("sourceBytes");
			_growMode = true;
			_tryMode = false;
			_chunkList = null;
			_chunk = new uint[Consts.ChunkCapacity];
			...
			fixed (byte* pSrcBytes = &sourceBytes[sourceIndex])
			{
				...
				int outSize = -1;
				if (sourceLength > 3 && compressionLevel != CompressionLevel.Store)
				{
		...
		/* Comment by Cang because not using now 2008-07-07 
		public unsafe int Compress(byte[] sourceBytes, int sourceIndex, int sourceLength,
			CompressionLevel compressionLevel, byte[] destinationBytes, int destinationIndex)
		{
			_tryMode = false;
			...
			{
				_tryMode = true;
				destinationBytes = _fakeDestinationBytes.Clone() as byte[];
				destinationIndex = 0;
				_dstBreak = sourceLength;
			}
			if (sourceLength == 0)
			{
				...
				{
					if (_dstBreak < 0)
						MCException.ThrowNoPlaceToStoreCompressedDataException();
