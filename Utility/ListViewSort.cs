...
namespace SiGlaz.Utility
{
	...
	public class ListViewColumnSorter : IComparer
	{
		...
		private bool	bStringFirst = false;
		...
		public ListViewColumnSorter(bool stringfirst):this()
		{
			bStringFirst = stringfirst;
		}
...
//			{
				...
				compareResult = 
					...
					listviewY.SubItems[ColumnToSort].Text,bStringFirst);
//			}
	...
	internal class NumberCaseInsensitiveComparer : CaseInsensitiveComparer
	{
		...
		{ 
			...
			{
				return base.Compare(System.Convert.ToInt64(x),
					System.Convert.ToInt64(y));
			}
			else
			{
		...
		public int Compare(object x, object y,bool stringfirst)
		{ 
			if(!stringfirst)
				return Compare(x,y);
			else
			{
				if ( (x is System.String) && (y is System.String) )
				{
					if(IsWholeNumber((string)x) && !IsWholeNumber((string)y) )
						return 1;
					else if(!IsWholeNumber((string)x) && IsWholeNumber((string)y) )
						return -1;
					else if(IsWholeNumber((string)x) && IsWholeNumber((string)y) )
						return base.Compare(System.Convert.ToInt64(x),
							System.Convert.ToInt64(y));
		...
		{ // use a regular expression to find out if string is actually a number
			...
			return strNumber!=null && strNumber.Trim()!=string.Empty && !objNotWholePattern.IsMatch(strNumber);
		}  
	}
}
