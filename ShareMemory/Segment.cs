...
//			}
		...
		public void WriteBytes(byte[] lpbyte)
		{
			// Ensure root object is serializable
			if( lpbyte==null )
			{
				throw new SharedMemoryException("byte array id null");
			}

//			if( currentSize < lpbyte.Length  )
//			{
//				throw new SharedMemoryException("The data to be stored is too large for the segment");
//			}
			...
			try
			{
				...
				bw.Write((long)lpbyte.Length);
				bw.Write(lpbyte,0,lpbyte.Length);
				bw.Flush();
				ms.Seek( 0, SeekOrigin.Begin );
				CopyStreamToSharedMemory(ms);
			}
