...
namespace SiGlaz.Windows
{
	...
	public enum ListBarGroupView
	{
		...
		/// </summary>
		[Description("The ListBar will display using large icons, with the text underneath.")]
		LargeIcons,
		/// <summary>
		/// The ListBar will display using small icons, with text to the left.
		/// </summary>
		[Description("The ListBar will display using small icons, with text to the left.")]
		SmallIcons,
		/// <summary>
	...
	public enum ListBarDrawStyle
	{
		...
		/// releases.
		/// </summary>
		[Description("The ListBar will draw using the style of the original Office releases.")]
		...
		/// (not implemented yet).
		/// </summary>
		[Description("The ListBar will draw using the Office 2003 style (not implemented yet).")]
	...
	{	
		...
		/// <summary>
		/// Gradient color start to draw nice caption control		
		/// </summary>
		private Color _colorStart = Color.AliceBlue;
		/// <summary>
		/// Gradient color end to draw nice caption control		
		/// </summary>
		private Color _colorEnd = Color.LightSkyBlue;
		...
		private Image normalBackground = null;
		...
		private Image hoveredBackground = null;
		public Image HoveredBackground
		{
			get { return hoveredBackground; }
			set { hoveredBackground = value; }
		}
		
		private ListBarItem selectedItem = null;
		...
		public Color GradientColorStart
		{
			...
			set 
			{
				...
				{
					_colorStart = value;
					this.ProcessGradientColorChanged();
				}
			}
		}
		...
		protected override void OnMouseDown( System.Windows.Forms.MouseEventArgs e)
		{
			...
			if (e.Button == MouseButtons.Left)
			{
				if (mouseTrack != null)
				{
					...
					{
						UnselectedAllItem();
						selectedItem = ((ListBarItem)mouseTrack);
						((ListBarItem)mouseTrack).SelectedItem = true;
		...
		private void UnselectedAllItem()
		{
			foreach (ListBarGroup group in groups)
				foreach (ListBarItem item in group.Items)
		...
		protected internal void GroupChanged(ListBarGroup group, bool addRemove)
		{
			...
			{	
				if (group != null)
				{
					...
					// update gradient colors for new group
					group.GradientColorStart = this._colorStart;
					group.GradientColorEnd = this._colorEnd;
				}
				DoResize();
		...
		#region Helpers
		private void ProcessGradientColorChanged()
		{
			if (groups == null || groups.Count == 0)
				return;

			// apply changing gradient colors for all children
			foreach (ListBarGroup group in groups)
			{
				group.GradientColorStart = this._colorStart;
				group.GradientColorEnd = this._colorEnd;
	...
	public class ListBarGroup : IMouseObject, ISerializable
	{
		...
		/// Gradient color start to draw nice caption control		
		/// </summary>
		public Color GradientColorStart = Color.AliceBlue;
		/// <summary>
		/// Gradient color end to draw nice caption control		
		/// </summary>
		public Color GradientColorEnd = Color.LightSkyBlue;
			...
			)
		{
			if (this.visible)
			{
				// save old rectangle to roll-back later
				Rectangle oldRectangle = this.rectangle;
				try
				{
					this.rectangle = new Rectangle(this.rectangle.Left-1, this.rectangle.Top-1, this.rectangle.Width + 1, this.rectangle.Height+1);
					...
					// Cong edited
					// it is able to change to edit gradient colors via design mode by adding some lines code according to public properties
					Color colorStart = this.GradientColorStart;
					Color colorEnd = this.GradientColorEnd;
					
					using (LinearGradientBrush brush = new LinearGradientBrush(this.rectangle, colorStart, colorEnd, LinearGradientMode.Vertical))
					{
						gfx.SmoothingMode = System.Drawing.Drawing2D.SmoothingMode.AntiAlias;
						gfx.FillRectangle(brush, this.rectangle);
					}
					...
					// Draw the border:
					Pen darkDarkPen = new Pen(CustomBorderColor.ColorDarkDark(colorEnd));
					Pen darkPen = new Pen(CustomBorderColor.ColorDark(colorEnd));
					Pen lightPen = new Pen(CustomBorderColor.ColorLight(colorEnd));
					Pen lightLightPen = new Pen(CustomBorderColor.ColorLightLight(colorEnd));
					Image backGround = null;

					if (this.mouseDown)
					{
						if (selectedBackground == null && owner.SelectedBackground == null)
						{
						...
						else
						{
							...
							if (selectedBackground == null)
								backGround = owner.SelectedBackground;

							Pen pen = new Pen(Color.Black);
							gfx.DrawImage(backGround, rectangle.X + 1, rectangle.Y + 1, rectangle.Width - 1, rectangle.Height - 1);
							gfx.DrawRectangle(pen, rectangle.X + 1, rectangle.Y + 1, rectangle.Width - 2, rectangle.Height - 2);
						}		
					}
					else if (this.MouseOver)
					{
						if (hoveredBackground == null && owner.HoveredBackground == null)
						{
							gfx.DrawLine(lightLightPen, 
								rectangle.Left, rectangle.Bottom - 2, 
								rectangle.Left, rectangle.Top);
						...
						else
						{
							...
							if (hoveredBackground == null)
								backGround = owner.HoveredBackground;
					...
					{
						if (normalBackground == null && owner.NormalBackground == null)
						{
							gfx.DrawLine(lightLightPen, 
								rectangle.Left, rectangle.Bottom - 2, 
						...
						else
						{
							...
							if (normalBackground == null)
								backGround = owner.NormalBackground;
	...
	public class ListBarItem : IComparable, IMouseObject, ISerializable
	{
		...
		{			
			...
			if (style == ListBarDrawStyle.ListBarDrawStyleOfficeXP)
			{
				...
				{
					SolidBrush highlight = null;

					if (selectedItem || mouseDown)
					{
						highlight = new SolidBrush(Color.FromArgb(184, 209, 248));
						gfx.FillRectangle(highlight, rcHighlight);
						gfx.DrawRectangle(new Pen(Color.Black), rcHighlight);
					}

					if (this.MouseOver)
					{
						highlight = new SolidBrush(Color.FromArgb(255, 238, 194));
						gfx.FillRectangle(highlight, rcHighlight);
						gfx.DrawRectangle(new Pen(Color.Black), rcHighlight);
					}
				}
